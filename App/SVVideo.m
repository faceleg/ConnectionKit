...
- (void)getPosterFrameFromQuickLook;
{
	...
	if (media)
	{
		...
		{
			// Only try to get thumbnail if the file can actually be found
		...
		{
			NSLog(@"Not able to create thumbnail for file at '%@' since it cannot be found", [mediaURL path]);
		}
...
- (void) setPosterFrameType:(PosterFrameType)aPosterFrameType
{
	...
	switch(aPosterFrameType)
	{
		case kPosterTypeChoose:
			...
			if (old == kPosterFrameTypeAutomatic)
			{
