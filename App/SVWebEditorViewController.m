...
- (void)loadPage:(KTPage *)page;
{
    ...
    // If no page, might be teardown time, so ditch content
    if (!page) [[self graphicsController] setContent:nil];
}
...
- (void)selectObjects:(NSArray *)objects inWebEditor:(WEKWebEditorView *)webEditor;
{
    ...
    for (id anObject in objects)
    {
        ...
        if ([[newItem HTMLElement] ks_isDescendantOfDOMNode:[[newItem HTMLElement] ownerDocument]])
        {
            ...
            if ([newItem shouldTrySelectingInline])
            {
     ...
     toPasteboard:(NSPasteboard *)pasteboard;
{
    ...
    // HACK for now, don't write inline images to pboard, so don't get any custom behaviour
    SVGraphic *graphic = [[items lastObject] representedObject];
    if ([graphic respondsToSelector:@selector(writeToPasteboard:)] &&
         (![graphic displayInline] || [graphic isKindOfClass:[SVRawHTMLGraphic class]]))
    {
        result = YES;
...
- (void)webEditor:(WEKWebEditorView *)sender handleNavigationAction:(NSDictionary *)actionInfo request:(NSURLRequest *)request;
{
    ...
    {
        SVSiteItem *page = [SVSiteItem siteItemForPreviewPath:relativePath inManagedObjectContext:[myPage managedObjectContext]];
        if (page)
        {
