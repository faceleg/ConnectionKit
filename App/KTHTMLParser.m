...
- (NSString *)stylesheetWithParameters:(NSString *)inRestOfTag scanner:(NSScanner *)inScanner
{
	...
	NSString *result = [NSString stringWithFormat:
		...
		[self resourceFilePathRelativeToCurrentPage:globalCSSFile]];
	
	
	// Then the base design's CSS file.
	result = [result stringByAppendingFormat:
		@"\r<link rel=\"stylesheet\" type=\"text/css\" href=\"%@\" title=\"%@\" />",
		[[self cache] valueForKey:@"cssURLPath"],
		[[[self cache] valueForKey:@"cssTitle"] escapedEntities]];
	...
	{
		result = [result stringByAppendingFormat:@"\r<link rel=\"stylesheet\" type=\"text/css\" href=\"%@\" />",
												 [self resourceFilePathRelativeToCurrentPage:aCSSFile]];
	...
	if ([self HTMLGenerationPurpose] == kGeneratingPreview)
	{
		...
		result = [result stringByAppendingFormat:@"\r<link rel=\"stylesheet\" type=\"text/css\" href=\"%@\" />",
												 [[NSURL fileURLWithPath:editingCSSPath] absoluteString]];
		...
		if (masterCSS)
		{
			result = [result stringByAppendingFormat:@"\r<style type=\"text/css\">\r%@\r</style>", masterCSS];
		}
	}
	else
	{
