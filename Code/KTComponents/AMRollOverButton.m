...
- (NSShadow *)textShadow
{
	return [(AMRollOverButtonCell *)[self cell] textShadow];
}

- (void)setTextShadow:(NSShadow *)newTextShadow
{
	[(AMRollOverButtonCell *)[self cell] setTextShadow:newTextShadow];
}
...
{
	[[self cell] setMouseOver:([[self window] isMainWindow] && nil == [[self window] attachedSheet])];
