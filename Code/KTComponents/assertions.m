...
#warning (Assertions enabled.  To disable, undefine OMNI_ASSERTIONS_ON or DEBUG, or define OMNI_FORCE_ASSERTIONS_OFF)
#else
...
static void OBLogAssertionFailure(const char *type, const char *expression, const char *file, unsigned int lineNumber)
{
	...
	char *substr = strstr(file, trunk);
	if (NULL != substr)
	{
		substr += strlen(trunk);	// skip past the trunk
	}
	else
	{
		substr = (char *) file;	// use whole file if we don't find
	}
    fprintf(stderr, "%s failed: requires '%s', %s:%d\n", type, expression, substr, lineNumber);
}
#endif
